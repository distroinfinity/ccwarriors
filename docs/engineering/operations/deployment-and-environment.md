...
Note: `apps/server/src/config.ts` zod-parses only the core set; `GATE_*`, `ADMIN_TOKEN`, `POSTHOG_*`, `NS_GUILD_ID`, `CLI_UPDATE_ENABLED`, `SIGIRO_*` are raw `process.env` reads at their call sites.
