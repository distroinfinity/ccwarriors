...
- `GET /profile/:login` is the **core** payload (identity, ranks, cost, `tokensAllTime`, rhythm, efficiency, github, owner) — additive-only and the web tolerates missing fields. The **insights** block moved to `GET /profile/:login/insights` (the union at the top level): `depth`, `economics`, `stack`, `tagline`, `featuredCardKeys`, `deckMonth`, `sampleSessions`, `windowDays`, `githubVerified`, craft/pillars, etc. — all optional/nullable, fetched in parallel by `useProfileInsights.ts`. The insights `locked.reason` is `no_consent | forging`. (A web build that calls `/insights` against an older server that lacks the route degrades to a locked region; deploy web before server.)
